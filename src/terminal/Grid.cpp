...
Coordinate Grid::resize(Size _newSize, Coordinate _currentCursorPos, bool _wrapPending)
{
    ...
    //     if (int const diff = historyLineCount() - maxHistoryLineCount_.value(); diff > 0)
    //         lines_.erase(lines_.begin(), next(lines_.begin(), diff));

    auto const growLines = [this, _newSize]() -> Coordinate {
        // Grow line count by splicing available lines from history back into buffer, if available,
        // or create new ones until screenSize_.height == _newSize.height.

        auto const extendCount = _newSize.height - screenSize_.height;
        ...
        screenSize_.height = _newSize.height;

        return Coordinate{rowsToTakeFromSavedLines, 0};
    };

    auto const shrinkLines = [this, _newSize](Coordinate _cursor) -> Coordinate {
        ...
        {
            auto const shrinkedLinesCount = screenSize_.height - _newSize.height;
            screenSize_.height = _newSize.height;
            clampHistory();
        ...
        else
        {
            ...
            lines_.resize(historyLineCount() + _newSize.height);
            screenSize_.height = _newSize.height;
    ...
    switch (crispy::strongCompare(_newSize.width, screenSize_.width))
    {
        case Comparison::Greater:
            for (Line& line : lines_)
                if (static_cast<int>(line.size()) < _newSize.width)
                    line.resize(_newSize.width);
            screenSize_.width = _newSize.width;
            cursorPosition.column += _wrapPending ? 1 : 0;
            break;
        case Comparison::Less:
            cursorPosition.column = min(cursorPosition.column, _newSize.width);
            screenSize_.width = _newSize.width;
            break;
        case Comparison::Equal:
            break;
    ...
    {
        case Comparison::Greater:
            cursorPosition += growLines();
            break;
        case Comparison::Less:
            cursorPosition += shrinkLines(cursorPosition);
            break;
        case Comparison::Equal:
            break;
    }
