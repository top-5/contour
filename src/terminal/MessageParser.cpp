...
void MessageParser::pass(char32_t _char)
{
    switch (state_)
    {
        case State::ParamKey:
        ...
        case State::Body:
            if (body_.size() < MaxBodyLength)
                body_.push_back(_char);
            // TODO: In order to avoid needless copies, I could pass the body incrementally back to the caller.
            break;
    }
}
    ...
    switch (state_)
    {
        ...
        case State::Body:
            if (!body_.empty() && body_[0] == '!')
            {
                ...
                crispy::base64::decode(next(begin(body_)), end(body_), &decoded[0]);
                body_ = move(decoded);
            }
            break;
    }
